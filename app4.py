...
# Load the original model and vectorizer
tfidf = pickle.load(open('vectorizer.pkl', 'rb'))
original_model = pickle.load(open('model.pkl', 'rb'))
...
def train_new_model(firebase_data):
    ...
    # Train a new model (using a simple model for demonstration; you can replace it with your preferred model)
...
def handle_new_data(input_sms, prediction, user_feedback):
    feedback_ref = db.collection('feedback')
    
    # Determine the final label to store
    final_label = "spam" if prediction == "ham" else "ham"
...
# Load Firebase data and train new model at startup
...
def predict():
    ...
    if is_message_in_dataset(input_sms, combined_data):
        ...
        original_prediction = original_model.predict(vector_input)[0]
...
def submit_feedback():
    ...
    handle_new_data(message, prediction, user_feedback)
